...
@Serializable
data class ProductFilterOptionsDto(
    val suppliers: List<String> = emptyList(),
)
...
@Serializable
data class OrderOptionsDto(
    val managers: List<LookupItemDto> = emptyList(),
    val products: List<LookupItemDto> = emptyList(),
...
class BackendClient(
    ...
    suspend fun getProducts(filter: ProductFilter): List<ProductListItem> {
        return httpClient.get(apiConfig.productsUrl) {
            url {
                ...
                parameters.append("supplierFilter", filter.supplierFilter)
    ...
    suspend fun getProductFilterOptions(): ProductFilterOptionsDto {
        val response = httpClient.get(apiConfig.productsFilterOptionsUrl) {
            authHeader()
        }
        if (!response.status.isSuccess()) {
            error("Не удалось загрузить фильтры каталога (HTTP ${response.status.value}).")
        }
        return response.body<ProductFilterOptionsDto>()
    }
    ...
    suspend fun getOrders(): List<OrderSummary> {
        val response = httpClient.get(apiConfig.ordersUrl) {
            authHeader()
        }
        if (!response.status.isSuccess()) {
            error(
                when (response.status.value) {
                    401, 403 -> "Недостаточно прав для просмотра заказов. Войдите как менеджер или администратор."
                    else -> "Не удалось загрузить заказы (HTTP ${response.status.value})."
                },
            )
        }
        return response.body<List<OrderSummaryDto>>().map { it.toModel() }
    }

    suspend fun getOrder(orderId: Int): OrderDetail? {
        val response = httpClient.get(apiConfig.orderUrl(orderId)) {
            authHeader()
        }
        return if (response.status.isSuccess()) {
            response.body<OrderDetailDto>().toModel()
        } else {
            null
        }
    }
    ...
    suspend fun nextOrderId(): Int {
        return httpClient.get(apiConfig.ordersNextIdUrl) {
            authHeader()
        }.body<Int>()
    }

    suspend fun getOrderOptions(): OrderOptionsDto {
        val response = httpClient.get(apiConfig.ordersOptionsUrl) {
            authHeader()
        }
        if (!response.status.isSuccess()) {
            error("Не удалось загрузить справочники заказа (HTTP ${response.status.value}).")
        }
        return response.body<OrderOptionsDto>()
    }

    suspend fun loadOrderEditorPayload(draft: OrderDetail): OrderEditorPayload {
        val options = getOrderOptions()
        return OrderEditorPayload(
            draft = draft,
            managers = options.managers.map { it.toModel() },
            products = options.products.map { it.toModel() },
            statuses = mergeStrings(options.statuses, draft.status),
            pickupAddresses = mergeStrings(options.pickupAddresses, draft.pickupAddress),
        )
    }

    private fun mergeStrings(options: List<String>, currentValue: String): List<String> {
    ...
    suspend fun saveOrder(draft: OrderDetail, isNew: Boolean): OrderDetail {
        val payload = draft.toDraftDto()
        val saved = if (isNew) {
            httpClient.post(apiConfig.ordersUrl) {
                authHeader()
                contentType(ContentType.Application.Json)
                setBody(payload)
            }.body<OrderDetailDto>()
        } else {
            httpClient.put(apiConfig.orderUrl(draft.id)) {
                authHeader()
                contentType(ContentType.Application.Json)
                setBody(payload)
            }.body<OrderDetailDto>()
    ...
    suspend fun deleteOrder(orderId: Int): Boolean {
        val response = httpClient.delete(apiConfig.orderUrl(orderId)) {
            authHeader()
        }
    ...
    suspend fun getProductOptions(): ProductOptionsDto {
        val response = httpClient.get(apiConfig.productsOptionsUrl) {
            authHeader()
        }
        if (!response.status.isSuccess()) {
            error(
                when (response.status.value) {
                    401, 403 -> "Недостаточно прав для загрузки справочников. Войдите как администратор."
                    else -> "Не удалось загрузить справочники (HTTP ${response.status.value})."
                },
            )
        }
        return response.body<ProductOptionsDto>()
    }
    ...
    private fun OrderSummaryDto.toModel() = OrderSummary(
        ...
        deliveryDate = deliveryDate?.let { LocalDateTime.parse(it) },
    ...
    private fun OrderDetailDto.toModel() = OrderDetail(
        id = id,
        customerName = customerName,
        managerId = managerId,
        managerName = managerName,
    ...
    private fun OrderDetail.toDraftDto() = OrderDraftDto(
        id = id,
        customerName = customerName,
        managerId = managerId,
        status = status,
    ...
    private fun LookupItemDto.toModel() = LookupItem(id = id, label = label)
}
